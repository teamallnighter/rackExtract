...
// Set number of inlets/outlets
Max.inlets = 3; // 0: commands, 1: track/device JSON, 2: metadata JSON
Max.outlets = 2; // 0: status/error, 1: rack data

// Handle any message type
Max.addHandler(Max.MESSAGE_TYPES.ALL, (handled, ...args) => {
    if (handled) return;
    
    const inlet = Max.inlet;
    const messageName = args[0];
    
    if (inlet === 0) {
        // Commands inlet
        handleCommand(messageName);
    } else if (inlet === 1) {
        // Track/Device JSON inlet
        handleTrackDevice(args.join(' '));
    } else if (inlet === 2) {
        // Metadata JSON inlet
        handleMetadata(args.join(' '));
    }
});
...
function handleCommand(command) {
    switch(command) {
        case 'analyze':
            ...
            break;
        case 'send':
            sendToServer().catch(err => Max.outlet('error', err.message));
            break;
        case 'clear':
            clearData();
            break;
        default:
            Max.outlet('error', `Unknown command: ${command}`);
    }
}

function handleTrackDevice(jsonString) {
    try {
...
function handleMetadata(jsonString) {
    try {
